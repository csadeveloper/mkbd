SELECT B.NAME, A.*
                     FROM (SELECT SUBACCOUNT, SUM(AMOUNTIDR) AS AMOUNTIDR
                           FROM (SELECT L.SUBACCOUNT,
                                 ...
                                 WHERE L.TRANS_PERIOD = TO_CHAR(TO_DATE(:genDate, 'yyyy-mm-dd'), 'yyyymm') AND L.TRANS_DUEDATE <= TO_DATE(:genDate, 'yyyy-mm-dd')
                                 ...
                                 WHERE ACCOUNT = '10401131'
                                   AND (TRANS_DATE >= TRUNC(TO_DATE(:genDate, 'yyyy-mm-dd'), 'MM') AND TRANS_DUEDATE <= TO_DATE(:genDate, 'yyyy-mm-dd'))
                                   AND TRANS_CURRENT = 1
