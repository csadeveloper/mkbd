SELECT 'Regular Netting Transactions (Non NG), Exclude Institution and House' AS BOARD, SUM(AMOUNT) AS TOTAL_AMOUNT
                           FROM (SELECT CL_CODE,
                                 ...
                                 FROM (SELECT CL_CODE,
                                       ...
                                       WHERE A.DEAL_DATE BETWEEN DENPASAR.CALCULATE_DUEDATE(TO_CHAR(TO_DATE(:genDate, 'yyyy-mm-dd'), 'yyyy-mm-dd'), -2) AND DENPASAR.CALCULATE_DUEDATE(TO_CHAR(TO_DATE(:genDate, 'yyyy-mm-dd'), 'yyyy-mm-dd'), 0)
                                         AND A.BOARD != 'NG'
