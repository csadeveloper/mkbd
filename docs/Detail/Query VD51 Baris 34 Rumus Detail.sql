...
FROM (SELECT CL_CODE,
      ...
      FROM (SELECT CL_CODE,
            ...
            WHERE A.DEAL_DATE BETWEEN DENPASAR.CALCULATE_DUEDATE(
                    TO_CHAR(TO_DATE('2026-04-21', 'yyyy-mm-dd'), 'yyyy-mm-dd'),
                    -2) AND DENPASAR.CALCULATE_DUEDATE(
                    TO_CHAR(TO_DATE('2026-04-21', 'yyyy-mm-dd'), 'yyyy-mm-dd'), 0)
              AND A.BOARD != 'NG'
      ...
      FROM DENPASAR.REGULAR A
      WHERE A.DEAL_DATE BETWEEN DENPASAR.CALCULATE_DUEDATE(TO_CHAR(TO_DATE('2026-04-21', 'yyyy-mm-dd'), 'yyyy-mm-dd'),
                                                           -2) AND DENPASAR.CALCULATE_DUEDATE(
              TO_CHAR(TO_DATE('2026-04-21', 'yyyy-mm-dd'), 'yyyy-mm-dd'), 0)
        AND A.BOARD = 'NG'
        AND A.CL_INVOICE_DUE > TO_DATE('2026-04-21', 'yyyy-mm-dd')
